...
# 2026-08-30 부터 자동 발행을 중단했다(애드센스 대응 — daily-draft.yml 주석 참조).
# 그래서 "오늘 초안이 없음"은 이제 정상 상태다. 재실행도, 알림도 하지 않는다.
#
# 이 자동 재실행은 스케줄 누락(8/27 은 8시간 지연, 8/28 은 미실행)을 메우려고 붙인 것인데,
# 발행을 멈춘 지금은 정확히 반대로 작동한다 — 껐다고 생각한 파이프라인을 감시가 매일 되살린다.
# 발행을 재개할 때 PAUSED=0 으로 되돌리면 예전 동작이 그대로 살아난다.
PAUSED=${PAUSED:-1}
RETRIGGERED=0

if [ "$PAUSED" = "1" ]; then
  echo "발행 일시중지 중 — 오늘 초안이 없어도 재실행·알림하지 않습니다"
  RETRIGGERED=1   # 아래 "초안 없음" 알림을 건너뛰기 위해
elif [ "$TODAY_MERGED" = "0" ] && [ "$INFLIGHT" = "0" ] && [ -z "$STALE" ]; then
